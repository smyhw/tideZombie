                ...
                if (configer.getStringList("area_list").size() != 0) {
                    ...
                    for (HashMap<String, Integer> line : area_list) {
                        if (p.getLocation().getBlockX() <= line.get("X_max") &&
                                p.getLocation().getBlockX() >= line.get("X_min") &&
                                p.getLocation().getBlockY() <= line.get("Y_max") &&
                                p.getLocation().getBlockY() >= line.get("Y_min") &&
                                p.getLocation().getBlockZ() <= line.get("Z_max") &&
                                p.getLocation().getBlockZ() >= line.get("Z_min")) {
                            inAnyArea = true;
                            break;
                        }
                    }
...
//				Helper.spawnMob(spawnLoc,p,tideID);
    ...
    public boolean init(DoMob tide, ConfigurationSection configer) {
        ...
        for (String name : WorldNameStringList) {
            World tmp1 = Tz.thisPlugin.getServer().getWorld(name);
            if (tmp1 == null) {
        ...
        if (configer.getStringList("area_list").size() != 0) {
            for (String line : configer.getStringList("area_list")) {
                ...
                if (tmp1.length != 3) {
                    Tz.loger.warning("尸潮<" + tide.tideID + ">的坐标范围<" + line + ">格式错误,不能识别");
                    continue;
                }
                int tmp6, tmp7;
                String[] tmp3 = tmp1[0].split("~");
                if (tmp3.length != 2) {
                    Tz.loger.warning("尸潮<" + tide.tideID + ">的X轴坐标范围<" + line + ">格式错误,不能识别");
                    continue;
                }
                try {
                    tmp6 = Integer.parseInt(tmp3[0]);
                    tmp7 = Integer.parseInt(tmp3[1]);
                } catch (NumberFormatException e) {
                    Tz.loger.warning("尸潮<" + tide.tideID + ">的X轴坐标范围<" + line + ">数字格式错误,不能识别");
                    continue;
                }
                if (tmp6 > tmp7) {
                    tmp2.put("X_max", tmp6);
                ...
                if (tmp4.length != 2) {
                    Tz.loger.warning("尸潮<" + tide.tideID + ">的Y轴坐标范围<" + line + ">格式错误,不能识别");
                    continue;
                }
                try {
                    tmp6 = Integer.parseInt(tmp4[0]);
                    tmp7 = Integer.parseInt(tmp4[1]);
                } catch (NumberFormatException e) {
                    Tz.loger.warning("尸潮<" + tide.tideID + ">的Y轴坐标范围<" + line + ">数字格式错误,不能识别");
                    continue;
                }
                if (tmp6 > tmp7) {
                    tmp2.put("Y_max", tmp6);
                ...
                if (tmp5.length != 2) {
                    Tz.loger.warning("尸潮<" + tide.tideID + ">的Z轴坐标范围<" + line + ">格式错误,不能识别");
                    continue;
                }
                try {
                    tmp6 = Integer.parseInt(tmp5[0]);
                    tmp7 = Integer.parseInt(tmp5[1]);
                } catch (NumberFormatException e) {
                    Tz.loger.warning("尸潮<" + tide.tideID + ">的Z轴坐标范围<" + line + ">数字格式错误,不能识别");
                    continue;
                }
                if (tmp6 > tmp7) {
                    tmp2.put("Z_max", tmp6);
        ...
        Utils.broadcastMessageToWorld(WorldList, configer.getString("start_message", "§b[§ctideZombie§b]§r:尸潮已经开始..."));
        return true;
    }

    @Override
    public boolean stop() {
        Utils.broadcastMessageToWorld(WorldList, configer.getString("end_message", "§b[§ctideZombie§b]§r:尸潮已经停止..."));
        return true;
    }
}
