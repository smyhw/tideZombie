...
public class DoMob extends BukkitRunnable {
	...
	public final String tideID;
	public  final ConfigurationSection configer;
	...
	public DoMob(JavaPlugin plugin,String tideID,String reason) {
		...
		for(String name: WorldNameStringList) {
			...
			if(tmp1 == null) {
				Tz.loger.warning("尸潮<"+tideID+">指定的世界<" + name + ">不存在");
				continue;
			}
			WorldList.add(tmp1);
		}
		...
		this.runTaskTimer(this.plugin, 0, configer.getInt("spawn_ticks", 10));
	...
	public void run() {
		for(World wd: WorldList) {
			for(Player p: wd.getPlayers()) {
				...
				Location spawnLoc = Helper.getSpwanLoc(p.getLocation(),configer);
				//如果没有找到合适的刷怪位置，不刷怪
				if(spawnLoc == null) {
					continue;
				}
